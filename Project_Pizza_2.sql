...
--------------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------------
# Questões individuais e testes
select * from v_pizza;
select * from tempo;
select * from produto;
select * from order_pizza;
select * from ingredientes_n;

select pizza_name, ingredientes1 -- Ingredientes por pizza
from pizza_ingredientes
where ingredientes1 not like 'n%' 
group by 1,2;

select distinct -- Most ans less used ingredientes by ingredientes
	p.ingredientes, 
	count(aux.ingredientes1) 
from produto p 
join aux_ingredientes aux on p.pizza_name = aux.pizza_name  
where p.ingredientes not like 'null' 
group by 1 
order by 2 asc;

select distinct -- pizza name by ingredientes
	p.pizza_name, 
	count(aux.ingredientes1)
from produto p 
join aux_ingredientes aux on p.pizza_name = aux.pizza_name  
...
select distinct -- all ingredientes by ingredientes
	p.ingredientes, 
	count(aux.ingredientes1)
from produto p 
join aux_ingredientes aux on p.pizza_name = aux.pizza_name  
...
group by 1 
order by 2 desc;
select * from all_ingredientes;
select * from produto;

select distinct -- connection between the 4 tables 
	o.id,
    p.ingredientes, 
	count(aux.ingredientes1) 
from order_pizza o 
	join produto p on o.id = p.id
	join tempo t on o.id = t.id
		join aux_ingredientes aux on p.pizza_name = aux.pizza_name
